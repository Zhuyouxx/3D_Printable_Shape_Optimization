...
double Calculate_Stresses_AD(int nv, double* points, int n_tri, int* triangles, int n_tet, int* tetrahedras, int n_sp, int* index_sp, double t,
    ...
    for (int s_i = 0; s_i < n; s_i++) {
        int id = (int)s_i / 3;
        Vector3d normal; normal << Normal[s_i - 2], Normal[s_i - 1], Normal[s_i];
        if (normal.norm() == 0.0) {
            grad_O_s[s_i - 2] = 0; grad_O_s[s_i - 1] = 0.0; grad_O_s[s_i] = 0.0;
            continue;
        }
        else {
            Vector3d grad_p; grad_p << grad_O_s[s_i - 2], grad_O_s[s_i - 1], grad_O_s[s_i];
            grad_p = (grad_p.dot(normal) / normal.dot(normal)) * normal;
            grad_O_s[s_i - 2] = grad_p[0]; grad_O_s[s_i - 1] = grad_p[1]; grad_O_s[s_i] = grad_p[2];
        }
