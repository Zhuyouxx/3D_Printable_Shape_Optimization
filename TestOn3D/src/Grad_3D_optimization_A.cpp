...
//    return CotTheta;
//}
//int smooth_grad(int np,int n_sp, int* index_sp, double *points,int n_tri, int* triangles, vector<double> &grad,double *Normal,double lambda) {
//    
...
//    double* la_bel_op = (double*)calloc(3 * n_sp, sizeof(double));
//    double* Area = (double*)calloc(np, sizeof(double));
//    //获取邻接点信息
//    for (int k = 0; k < n_tri; k++) {
//        int v1 = triangles[3 * k];
//        int v2 = triangles[3 * k + 1];
//        int v3 = triangles[3 * k + 2];
//        Vector3d p_i(points[3 * v1], points[3 * v1 + 1], points[3 * v1 + 2]);
//        Vector3d p_j(points[3 * v2], points[3 * v2 + 1], points[3 * v2 + 2]);
//        Vector3d p_k(points[3 * v3], points[3 * v3 + 1], points[3 * v3 + 2]);
//        Vector3d tri_edge1 = p_j - p_i;
//        Vector3d tri_edge2 = p_k - p_i;
//        // 计算叉积
//        Vector3d crossProduct = tri_edge1.cross(tri_edge2);
//
//        // 计算面积
//        double area_tri = 0.5 * crossProduct.norm();
//        Area[v1] += area_tri;        Area[v2] += area_tri;        Area[v3] += area_tri;
//        // 更新邻接关系
...
//        Edge edge1(v1, v2);  Edge edge2(v2, v3); Edge edge3(v3, v1);
//        edgeToTriangles[edge1].push_back(k);
//        edgeToTriangles[edge2].push_back(k);
//        edgeToTriangles[edge3].push_back(k);
//
//    }
//    for (int i = 0; i < n_sp; i++) {
//        int id = index_sp[i];
//        for (const auto& jd : adjacency[id]) {
//            Edge edge(id, jd);
//            int t1 = edgeToTriangles[edge][0];
//            int t2 = edgeToTriangles[edge][1];
//            int alpha_id, belta_id;
//            for (int k = 0; k < 3; k++) {
//                if (triangles[3 * t1 + k] != id && triangles[3 * t1 + k] != jd)
//                    alpha_id = triangles[3 * t1 + k];
//                if (triangles[3 * t2 + k] != id && triangles[3 * t2 + k] != jd)
//                    belta_id = triangles[3 * t2 + k];
//            }
//            Vector3d p_i(points[3 * id], points[3 * id + 1], points[3 * id + 2]);
//            Vector3d p_j(points[3 * jd], points[3 * jd + 1], points[3 * jd + 2]);
//            Vector3d p_alpha_id(points[3 * alpha_id], points[3 * alpha_id + 1], points[3 * alpha_id + 2]);
//            Vector3d p_belta_id(points[3 * belta_id], points[3 * belta_id + 1], points[3 * belta_id + 2]);
//            Vector3d t1_vec1 = p_i - p_alpha_id;
//            Vector3d t1_vec2 = p_j - p_alpha_id;
//            Vector3d t2_vec1 = p_i - p_belta_id;
//            Vector3d t2_vec2 = p_j - p_belta_id;
//            double cot_alpha = Cotangent(t1_vec1, t1_vec2);
//            double cot_belta = Cotangent(t2_vec1, t2_vec2);
//            la_bel_op[3 * i] += (cot_alpha + cot_belta) * (grad[3 * jd] - grad[3 * id]);
//            la_bel_op[3 * i + 1] += (cot_alpha + cot_belta) * (grad[3 * jd + 1] - grad[3 * id + 1]);
//            la_bel_op[3 * i + 2] += (cot_alpha + cot_belta) * (grad[3 * jd + 2] - grad[3 * id + 2]);
//            //cout << "cot_alpha + cot_belta : " << cot_alpha + cot_belta << " || " << grad[3 * jd] - grad[3 * id] << endl;
//            //cout << "cot_alpha + cot_belta : " << cot_alpha + cot_belta << " || " << grad[3 * jd + 1] - grad[3 * id + 1] << endl;
//            //cout << "cot_alpha + cot_belta : " << cot_alpha + cot_belta << " || " << grad[3 * jd + 2] - grad[3 * id + 2] << endl;
//        }
//        //cout << "id : " << id << " || " << la_bel_op[3 * i] << " , " << la_bel_op[3 * i + 1] << " , " << la_bel_op[3 * i + 2] << endl;
//        la_bel_op[3 * i] /= 2 * Area[ id];
//        la_bel_op[3 * i + 1] /= 2 * Area[id];
//        la_bel_op[3 * i + 2] /= 2 * Area[id];
//        //cout << "id : " << id << " || " << la_bel_op[3 * i] << " , " << la_bel_op[3 * i + 1] << " , " << la_bel_op[3 * i + 2] << endl;
//        //cout << "id : " << id << " || " << grad[3 * i] << " , " << grad[3 * i + 1] << " , " << grad[3 * i + 2] << endl;
//        //cout << "--------------------------------------------------------------------" << endl;
//        Vector3d normal; normal << Normal[3 * id], Normal[3 * id + 1], Normal[3 * id + 2];
//        Vector3d grad_p; grad_p << grad[3 * id] + lambda * la_bel_op[3 * i], grad[3 * id + 1] + lambda * la_bel_op[3 * i + 1], grad[3 * id + 2] + lambda * la_bel_op[3 * i + 2];
//        grad_p = (grad_p.dot(normal) / normal.dot(normal)) * normal;
//        grad[3 * id] = grad_p[0]; grad[3 * id + 1] = grad_p[1]; grad[3 * id + 2] = grad_p[2];
//        
//    }
//}
...
int stress_grad(string filein, string fileout, std::vector<double>& vec_grad, bool info_f_g) {
    double lambda = 1e-5; //smooth lambda
    double edge_length;
    //string model = "cone";
    string model = filein;
    ...
    for (int iteration = 0; iteration < 1; iteration++) {
        ...
        CppAD::AD<double> O = Calculate_O(t[0], sparse_Stress, n_tet, log_V);
        while (O < -1e5) {
            ...
            O = Calculate_O(t[0], sparse_Stress, n_tet, log_V);
        }
        if (intersect) {
        ...
        while (loop) {
            ...
            //smooth_grad(np,n_sp, Index_of_sp, points, n_tri, triangles, grad_s,Normals, lambda);
            //vec_grad = grad_s;
            sparse_Stress = Calculate_Stresses(np, points, n_sp, Index_of_sp, n_tri, triangles, n_tet, tetrahedras, p_stress, solver_LU, intersect, log_V);
            if (intersect) {
                cout << "There is a intersection!" << endl;
            ...
            if (maxVal * alpha < edge_length * 0.0001) {
