...
T Calculate_log_A(const vector<T>& points, int n_tet, int* tetrahedras) {
    T log_A = 0.0;
    for (int k = 0; k < n_tet; k++) {
        T p1_x, p1_y, p1_z, p2_x, p2_y, p2_z, p3_x, p3_y, p3_z, p4_x, p4_y, p4_z;
        int t1, t2, t3, t4;
        Matrix<T, 4, 4> Lambda;
        t1 = tetrahedras[k * 4]; t2 = tetrahedras[k * 4 + 1]; t3 = tetrahedras[k * 4 + 2]; t4 = tetrahedras[k * 4 + 3];
        ...
        Lambda << 1, p1_x, p1_y, p1_z,
            ...
            1, p3_x, p3_y, p3_z,
            1, p4_x, p4_y, p4_z;
        T Volume = Lambda.determinant() / 6.0;
        log_A += log(Volume);
    }
    return log_A;
}
...
Eigen::SparseMatrix<double> Calculate_Stresses_for_test(int nv, double* points, double* vertices, int n_sp, int* index_sp, int n_tri, int* triangles, int n_tet, int* tetrahedras,
...
double Smooth_energy(double lambda, int nv, double* points, int n_tri, int* triangles, int n_sp, int* index_sp) {
    double smooth_energy = 0.0;
    vector<int> neighbor_num(n_sp);
    vector<Vector3d> smooth_j(n_sp, Vector3d::Zero());
    for (int k = 0; k < n_tri; k++) {
        int t1 = triangles[3 * k], t2 = triangles[3 * k + 1], t3 = triangles[3 * k + 2];
        int new_t1 = Old_2_new[t1], new_t2 = Old_2_new[t2], new_t3 = Old_2_new[t3];
        Vector3d p_1(points[3 * t1], points[3 * t1 + 1], points[3 * t1 + 2]);
        Vector3d p_2(points[3 * t2], points[3 * t2 + 1], points[3 * t2 + 2]);
        Vector3d p_3(points[3 * t3], points[3 * t3 + 1], points[3 * t3 + 2]);
        smooth_j[new_t1] -= p_2 + p_3;
        smooth_j[new_t2] -= p_1 + p_3;
        smooth_j[new_t3] -= p_2 + p_1;
        neighbor_num[new_t1] += 2;
        neighbor_num[new_t2] += 2;
        neighbor_num[new_t3] += 2;
    }
    for (int i = 0; i < n_sp; i++) {
        int id = index_sp[i];
        Vector3d v_i(points[3 * id], points[3 * id + 1], points[3 * id + 2]);
        smooth_j[i] = v_i + smooth_j[i] / neighbor_num[i];
        smooth_energy += smooth_j[i].squaredNorm();
    }
    smooth_energy *= lambda;
    cout << "The energy of smooth : " << smooth_energy << " || lambda : " << lambda << endl;
...
CppAD::AD<double> Calculate_O(const CppAD::AD<double> t, Eigen::SparseMatrix<double> stress, int n_tet, double log_V,double smooth_E) {
    ...
    O += t;
    O -= A_mu * CppAD::AD<double>(log_V);
    O += Smooth_mu * CppAD::AD<double>(smooth_E);
...
double Calculate_Stresses_AD(int nv, double* points, int n_tri, int* triangles, int n_tet, int* tetrahedras, int n_sp, int* index_sp, double t,
    ...
    //    vertices[3 * id] = vertices_sp[3 * i]; vertices[3 * id + 1] = vertices_sp[3 * i + 1]; vertices[3 * id + 2] = vertices_sp[3 * i + 2];
    //}
    //CppAD::AD<double> log_volume = Calculate_log_A(vertices, n_tet, tetrahedras);
    //vector<CppAD::AD<double>> log_volume_vec(1);
    //log_volume_vec[0] = log_volume;
    //CppAD::ADFun<double> V_func(vertices_sp, log_volume_vec);    // 创建 ADFun 对象
    //jac_V = V_func.Jacobian(vertices_sp_x);
...
double log_A_AD(int nv, double* points, int n_tri, int* triangles, int n_tet, int* tetrahedras, int n_sp, int* index_sp, vector<double>& grad_s) {
    int n = 3 * nv;
    vector<double> grad_a(n);
    double* Normal;
    Build_N(nv, n_tri, points, triangles, n_sp, index_sp, &Normal);
    std::vector<double> jac_V;
    auto start_time_Np = std::chrono::high_resolution_clock::now();
    //calculate Grad_K
    vector<CppAD::AD<double>> vertices(n);
    ...
    CppAD::AD<double> log_volume = A_mu * Calculate_log_A(vertices, n_tet, tetrahedras);
    vector<CppAD::AD<double>> log_volume_vec(1);
    log_volume_vec[0] = log_volume;
    CppAD::ADFun<double> V_func(vertices_sp, log_volume_vec);    // 创建 ADFun 对象
    jac_V = V_func.Jacobian(vertices_sp_x);
    ...
    for (int s_i = 0; s_i < n_sp; s_i++) {
        ...
        for (int s_i_cord = 0; s_i_cord < 3; s_i_cord++) {
            int s_i_old_i = 3 * s_i_old + s_i_cord;
            int s_i_i = 3 * s_i + s_i_cord;
            grad_a[s_i_old_i] += -jac_V[s_i_i];
            //grad_O_s[s_i_old_i] += O_mu * (part2);
            vertices[s_i_old_i] = CppAD::AD<double>(points[s_i_old_i]);
        }
        Vector3d grad_p; grad_p << grad_a[3 * s_i_old], grad_a[3 * s_i_old + 1], grad_a[3 * s_i_old + 2];
        grad_p = (grad_p.dot(normal) / normal.dot(normal)) * normal;
        grad_s[3 * s_i_old] += grad_p[0]; grad_s[3 * s_i_old + 1] += grad_p[1]; grad_s[3 * s_i_old + 2] += grad_p[2];
        //grad_s[3 * s_i_old] = grad_p[0]; grad_s[3 * s_i_old + 1] = grad_p[1]; grad_s[3 * s_i_old + 2] = grad_p[2];
        grad_norm += grad_p.norm();
    }
    cout << "The norm of grad_A : " << grad_norm << endl;
    return 1.0;
}
...
void Smooth_AD(double lambda,int nv, double* points, int n_tri, int* triangles, int n_sp, int* index_sp, vector<double>& grad_s) {
    ...
    vector<Vector3d> smooth_grad(n_sp, Vector3d::Zero());
    vector<int> neighbor_num(n_sp);
    for (int k = 0; k < n_tri; k++) {
        int t1 = triangles[3 * k], t2 = triangles[3 * k + 1], t3 = triangles[3 * k + 2];
        int new_t1 = Old_2_new[t1], new_t2 = Old_2_new[t2], new_t3 = Old_2_new[t3];
        ...
        Vector3d p_2(points[3 * t2], points[3 * t2 + 1], points[3 * t2 + 2]);
        Vector3d p_3(points[3 * t3], points[3 * t3 + 1], points[3 * t3 + 2]);
        smooth_grad[new_t1] -= p_2 + p_3;
        smooth_grad[new_t2] -= p_1 + p_3;
        smooth_grad[new_t3] -= p_2 + p_1;
        neighbor_num[new_t1] += 2;
        neighbor_num[new_t2] += 2;
        neighbor_num[new_t3] += 2;
    }
    for (int i = 0; i < n_sp; i++) {
        int id = index_sp[i];
        //Vector3d normal_id; normal_id << Normal[3 * id], Normal[3 * id + 1], Normal[3 * id + 2];
        Vector3d v_i(points[3 * id], points[3 * id + 1], points[3 * id + 2]);
        smooth_grad[i] = lambda * 2.0 * (v_i + smooth_grad[i] / neighbor_num[i]*1.0);
        //smooth_grad[i] = (smooth_grad[i].dot(normal_id) / normal_id.dot(normal_id)) * normal_id;
        //grad_s[3 * id] = grad_s[3 * id] + lambda * smooth_grad[i][0];
        //grad_s[3 * id + 1] = grad_s[3 * id + 1] + lambda * smooth_grad[i][1];
        //grad_s[3 * id + 2] = grad_s[3 * id + 2] + lambda * smooth_grad[i][2];
        grad_s[3 * id] += smooth_grad[i][0];
        grad_s[3 * id + 1] += smooth_grad[i][1];
        grad_s[3 * id + 2] += smooth_grad[i][2];
        smooth_norm += smooth_grad[i].norm();
    }
    cout << "The norm of smooth grad : " << smooth_norm << " || lambda : " << lambda << endl;
}

double smooth_energy_Ad(double lambda,int nv, double* points, int n_tri, int* triangles, int n_sp, int* index_sp, vector<double>& grad_s) {
    int n = 3 * nv;
    vector<double> grad_a(n);
    double* Normal;
    std::vector<double> jac_V;
    ...
    CppAD::AD<double> smooth_energy = 0.0;
    vector<CppAD::AD<double>> neighbor_num(n_sp);
    vector<Vector3d_AD> smooth_j(n_sp, Vector3d_AD::Zero());
    for (int k = 0; k < n_tri; k++) {
        int t1 = triangles[3 * k], t2 = triangles[3 * k + 1], t3 = triangles[3 * k + 2];
        int new_t1 = Old_2_new[t1], new_t2 = Old_2_new[t2], new_t3 = Old_2_new[t3];
        Vector3d_AD p_1(vertices[3 * t1], vertices[3 * t1 + 1], vertices[3 * t1 + 2]);
        Vector3d_AD p_2(vertices[3 * t2], vertices[3 * t2 + 1], vertices[3 * t2 + 2]);
        Vector3d_AD p_3(vertices[3 * t3], vertices[3 * t3 + 1], vertices[3 * t3 + 2]);
        smooth_j[new_t1] -= p_2 + p_3;
        smooth_j[new_t2] -= p_1 + p_3;
        smooth_j[new_t3] -= p_2 + p_1;
        neighbor_num[new_t1] += 2;
    ...
    for (int i = 0; i < n_sp; i++) {
        int id = index_sp[i];
        Vector3d_AD v_i(vertices[3 * id], vertices[3 * id + 1], vertices[3 * id + 2]);
        smooth_j[i] = v_i + smooth_j[i] / ( neighbor_num[i]);
        cout << smooth_j[i] << endl;
        smooth_energy += smooth_j[i].squaredNorm();
    }
    smooth_energy *= lambda;
    vector<CppAD::AD<double>> smooth_energy_vec(1);
    smooth_energy_vec[0] = smooth_energy;
    CppAD::ADFun<double> V_func(vertices_sp, smooth_energy_vec);    // 创建 ADFun 对象
    jac_V = V_func.Jacobian(vertices_sp_x);
    //calculate grad_sigma(s)
    ...
    cout << "The norm of grad_smooth_energy : " << grad_norm << endl;
    return 1.0;
}

double Cotangent(const Eigen::Vector3d& a, const Eigen::Vector3d& b) {
    double CotTheta = a.dot(b) / a.cross(b).norm();
    return CotTheta;
}

int smooth_grad(int np,int n_sp, int* index_sp, double *points,int n_tri, int* triangles, vector<double> &grad,double lambda) {
    double* Normal;
    Build_N(np, n_tri, points, triangles, n_sp, index_sp, &Normal);
    double smooth_norm = 0.0;
    vector<Vector3d> grad_new(n_sp, Vector3d::Zero());
    vector<int> neighbor_num(n_sp);
    for (int k = 0; k < n_tri; k++) {
        int t1 = triangles[3 * k], t2 = triangles[3 * k + 1], t3 = triangles[3 * k + 2];
        int new_t1 = Old_2_new[t1], new_t2 = Old_2_new[t2], new_t3 = Old_2_new[t3];
        ...
        //Vector3d p_2(points[3 * t2], points[3 * t2 + 1], points[3 * t2 + 2]);
        //Vector3d p_3(points[3 * t3], points[3 * t3 + 1], points[3 * t3 + 2]);
        Vector3d grad_p_1(grad[3 * t1], grad[3 * t1 + 1], grad[3 * t1 + 2]);
        Vector3d grad_p_2(grad[3 * t2], grad[3 * t2 + 1], grad[3 * t2 + 2]);
        Vector3d grad_p_3(grad[3 * t3], grad[3 * t3 + 1], grad[3 * t3 + 2]);
        grad_new[new_t1] -= grad_p_2 + grad_p_3;
        grad_new[new_t2] -= grad_p_1 + grad_p_3;
        grad_new[new_t3] -= grad_p_2 + grad_p_1;
        neighbor_num[new_t1] += 2;
        neighbor_num[new_t2] += 2;
        neighbor_num[new_t3] += 2;
    }
    for (int i = 0; i < n_sp; i++) {
        ...
        Vector3d normal_id; normal_id << Normal[3 * id], Normal[3 * id + 1], Normal[3 * id + 2];
        Vector3d v_i(points[3 * id], points[3 * id + 1], points[3 * id + 2]);
        grad_new[i] = lambda * 2.0 * (v_i + grad_new[i] / neighbor_num[i] * 1.0);
        grad_new[i] = (grad_new[i].dot(normal_id) / normal_id.dot(normal_id)) * normal_id;
        grad[3 * id] += grad_new[i][0];
        grad[3 * id + 1] += grad_new[i][1];
        grad[3 * id + 2] += grad_new[i][2];
        smooth_norm += grad_new[i].norm();
    }
    cout << "The norm of smooth grad : " << smooth_norm << " || lambda : " << lambda << endl;
}
...
int extract_info_with_old_grad(string filename, int n_sp, int* Index_of_sp, int n_tet, int* tetrahedras, int n_tri, int* triangles, double* points, SparseMatrix<double> sparse_stress,
    ...
    if (grad_old.size() > 0) {
        save_str = save_path + filename + "_grad_old.txt";
        save_file = const_cast<char*>(save_str.c_str());
        file.open(save_file);
        if (!file.is_open()) {
            std::cerr << "Failed to open file." << std::endl;
        ...
        for (int i = 0; i < n_sp; i++)
            file << grad_old[3 * Index_of_sp[i]] << endl << grad_old[3 * Index_of_sp[i] + 1] << endl << grad_old[3 * Index_of_sp[i] + 2] << endl;
        file.close();
    }
...
int stress_grad(string filein, string fileout, std::vector<double>& vec_grad, bool info_f_g) {
    double lambda = 1e6; //smooth lambda
    double edge_length;
    //string model = "cone";
    string model = filein;
    ...
    //string model_name = "../IO/hanging_ball_mmg";
    //std::string path = "../IO/";
    ...
    for (int iteration = 0; iteration < 1; iteration++) {
        ...
        double log_V;
        double smooth_E;
        Eigen::SparseMatrix<double> sparse_Stress = Calculate_Stresses(np, points, n_sp, Index_of_sp, n_tri, triangles, n_tet, tetrahedras, p_stress, solver_LU, intersect, log_V);
        cout << sparse_Stress.coeffRef(0, 0) << endl;
        smooth_E = Smooth_energy(lambda, np, points, n_tri, triangles, n_sp, Index_of_sp);
        CppAD::AD<double> O = Calculate_O(t[0], sparse_Stress, n_tet, log_V, smooth_E);
        while (O < -1e5) {
            std::cout << "Invalid Initial!!" << endl;
            t[0] *= 2.0; t_x[0] = Value(t[0]);
            sparse_Stress = Calculate_Stresses(np, points, n_sp, Index_of_sp, n_tri, triangles, n_tet, tetrahedras, p_stress, solver_LU, intersect, log_V);
            smooth_E = Smooth_energy(lambda, np, points, n_tri, triangles, n_sp, Index_of_sp);
            O = Calculate_O(t[0], sparse_Stress, n_tet, log_V, smooth_E);
            std::cout << O << endl;
        }
        if (intersect) {
        ...
        while (loop) {
            ...
            vector<double> grad_s_old(n);
            Calculate_Stresses_AD(np, points, n_tri, triangles, n_tet, tetrahedras, n_sp, Index_of_sp, t_x[0], p_stress, grad_s, solver_LU);
            for (int i = 0; i < n; i++)
                grad_s_old = grad_s;
            //smooth_grad(np, n_sp, Index_of_sp, points, n_tri, triangles, grad_s, lambda);
            //log_A_AD(np, points, n_tri, triangles, n_tet, tetrahedras, n_sp, Index_of_sp, grad_s);

            Smooth_AD(lambda, np, points, n_tri, triangles, n_sp, Index_of_sp, grad_s);
            //smooth_energy_Ad(lambda, np, points, n_tri, triangles, n_sp, Index_of_sp, grad_s_old);
            std::ostringstream stream_index;
            // 设置宽度为3，左侧填充0
            stream_index << std::setw(3) << std::setfill('0') << iter;
            string file_out_index = fileout + stream_index.str();
            if (info_f_g)
                //extract_info(file_out_index, n_sp, Index_of_sp, n_tet, tetrahedras, n_tri, triangles, x, sparse_Stress, f, grad_s);
                ...
                extract_info(file_out_index, n_sp, Index_of_sp, n_tet, tetrahedras, n_tri, triangles, x, sparse_Stress);
            Build_N(np, n_tri, points, triangles, n_sp, Index_of_sp, &Normals);
            ...
            if (maxVal * alpha < edge_length * 1e-6) {
            ...
            while (maxVal * alpha > edge_length * 0.5) {
                alpha *= gamma;
                loop_s_time++;
            }
            ...
            while (true) {
                ...
                sparse_Stress = Calculate_Stresses(np, points, n_sp, Index_of_sp, n_tri, triangles, n_tet, tetrahedras, p_stress, solver_LU_new, intersect, log_V);
                smooth_E = Smooth_energy(lambda, np, points, n_tri, triangles, n_sp, Index_of_sp);
                O = Calculate_O(t[0], sparse_Stress, n_tet, log_V, smooth_E);
                if (O< last_O_s && O > -1e5 && !intersect) {
                    ...
                    alpha *= gamma;
                    if (maxVal * alpha < edge_length * 1e-8) {
                        cout << "|-The maximum update step size is: " << maxVal * alpha << ", which is much smaller than the edge length, recalculate the pressure p.-|" << endl;
                        break;
                    }
                }

                cout << "||max of O_s: " << maxVal << "   || alpha : " << alpha << endl;
                auto end_time_iter = std::chrono::high_resolution_clock::now();
        ...
        extract_surface(n_sp, Index_of_sp, n_tri, triangles, points, &new_points, &new_triangles);
        tet_remesh_mmg_Update_Points(mmgMesh, mmgSol, edge_length, points, n_sp, Index_of_sp);
        //read_mesh(mmgMesh, mmgSol, points, triangles, tetrahedras);
    }
