...
int tet_mesh(double& ideal_edge_length, string filename, const std::string& output_name , double rel , bool m_f , bool smooth)
{
    ...
    params.input_path = filename;
    if (output_name.empty()) 
        params.output_path = params.input_path.substr(0, params.input_path.size() - 4) + "_remesh.mesh";
    else
        params.output_path = output_name;
    params.tag_path = "";// , "Tag input faces for Boolean operation."
    //    const int UNION = 0;
    //    const int INTERSECTION = 1;
    //    const int DIFFERENCE = 2;
    int         boolean_op = -1;//Boolean operation: 0: union, 1: intersection, 2: difference
    std::string csg_file = "";
    params.ideal_edge_length_rel = rel; //"ideal_edge_length = diag_of_bbox * L. (double, optional, default: 0.05)"
    params.eps_rel = 1e-3;// "epsilon = diag_of_bbox * EPS. (double, optional, default: 1e-3)
    params.stop_energy = 10;// , "Stop optimization when max energy is lower than this."
    params.log_path = "";//Log info to given file.
    params.log_level = 6;//Log level (0 = most verbose, 6 = off).
    params.is_quiet = true;//Mute console output. (optional)
    // nobinary, "export meshes as ascii"
    // nocolor, "don't export color"
    params.smooth_open_boundary = smooth;//, "Smooth the open boundary."
    params.manifold_surface = m_f;// "Force the output to be manifold.
    params.coarsen = false;// "Coarsen the output as much as possible."
    params.disable_filtering = false;// "Disable filtering out outside elements."
    params.use_floodfill = false;// "Use flood-fill to extract interior volume."
    params.use_general_wn = false;// "Use general winding number."
    params.use_input_for_wn = false;// "Use input surface for winding number.");

    std::string background_mesh = "";//Background mesh for sizing field (.msh file).
    ...
    //int intersect_count = std::count(is_face_inserted.begin(), is_face_inserted.end(), true);
    ...
    // fortest
    ideal_edge_length =  mesh.params.ideal_edge_length;
